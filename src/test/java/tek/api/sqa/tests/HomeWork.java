...
public class HomeWork extends DatabaseConfig {
...
//		String returnAllAccountQueryWithId = "select * from primary_person where id = " + accountId;
		...
		String returnAllAccountQuery = "select * from primary_person order by id desc limit 1";
		ResultSet resultSet = runQuery(returnAllAccountQuery);
//		while (resultSet.next()) {
//			if(accountId == resultSet.getLong("id")){
//				queryResultId = accountId;
//			}
//		}
...
//		Assert.assertEquals(actualPrimaryPersonId, queryResultId);
		...
		String validateAccount = "select * from primary_person where id = " + queryResultId;
...
//			Assert.assertEquals(responseBody.getDateOfBirth(), expectedDateOfBirth);
			...
			Assert.fail("Test Fail validate account test did not return result for id " + queryResultId);
		}
